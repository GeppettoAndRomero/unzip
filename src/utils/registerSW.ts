...
export async function registerServiceWorker(): Promise<ServiceWorkerRegistration | null> {
  ...
  try {
    // Service Workerを登録（/unzip/sw.js, scope=/unzip/）。
    // scope がツールの全 locale ページ（/unzip/, /unzip/ja/ …）を被覆する。
    const registration = await navigator.serviceWorker.register('/unzip/sw.js', {
      scope: '/unzip/',
    });
