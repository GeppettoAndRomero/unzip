...
test.describe('covenants', () => {
  ...
  test('links to related tools and its own engineering-notes article (#177)', async ({ page }) => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      expect(href, `related card ${i} is not a self-link`).not.toContain('/unzip/');
    ...
    if (await blogLink.count()) {
      await expect(blogLink).toHaveAttribute('href', /^https:\/\/runlocally\.app\/blog\/unzip\/$/);
    }
  });
  ...
  test('keeps no input data in the page URL after opening an archive (#7)', async ({ page }) => {
    await page.goto('/unzip/');
  ...
  test('opens an archive offline after the first online visit (#2)', async ({ page }) => {
    ...
    await page.reload();
    await waitReady(page);
    // Open an archive online so the zip chunk is fetched through the SW and cached.
    await openSampleZip(page);
    ...
    try {
      ...
      await waitReady(page);
      await openSampleZip(page); // extract with no network at all
    } finally {
